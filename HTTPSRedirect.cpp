...
void HTTPSRedirect::fetchData(bool disp, bool header)
{
  ...
  while (connected())
  {
    ...
    if (disp)
    {
      ...
      // read the timers for the relays
      if (line.startsWith("notification"))
      {
        Serial.println("Haha! Here are the timers values: " + line);
        int commaIndex1 = line.indexOf(',');

        String notificationStatusString = line.substring(commaIndex1 + 1);
        Serial.println("notification read: " + notificationStatusString);
        notificationStatus = notificationStatusString.toInt();
